  ...
  for (const vp of VIEWPORTS) {
    ...
    if (vp.isMobile) {
      ...
      try {
        ...
        const touchTargetAudit = await page.evaluate(() => {
          const tabLinks = document.querySelectorAll('div.fixed.bottom-0 a');
          let tabResults = [];
        ...
        const allTabsErgonomic = touchTargetAudit.tabResults.length > 0 && touchTargetAudit.tabResults.every(t => t.height >= 44);
        if (allTabsErgonomic) {
          console.log(`[PASS] Mobile Bottom Bar Touch Targets (${vp.name}): All ${touchTargetAudit.tabResults.length} tabs have height >= 44px (compliant with WCAG touch standard)`);
          passedTests++;
        } else {
    ...
    try {
      const zIndexAudit = await page.evaluate(() => {
        ...
        const chatLauncher = document.querySelector('div.fixed.bottom-24, div.fixed.bottom-6');
