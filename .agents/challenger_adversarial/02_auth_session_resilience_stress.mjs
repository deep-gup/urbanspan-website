...
async function runApiAuthResilienceHarness() {
  ...
  try {
    ...
    for (const badToken of corruptedTokens) {
      ...
      // Should reject with 401, 403, or json error (not crash 500)
      if (res.status === 500) {
        allHandled = false;
  ...
  // 5. Wrong Password / Non-existent User
  totalTests++;
  try {
    const res = await fetch(`${API_BASE}/api/external/customers/login`, {
      method: 'POST',
    ...
    const data = await res.json();
    if (res.status === 400 || res.status === 401 || (res.status === 200 && data.success === false)) {
      console.log(`[PASS] Wrong password rejected gracefully with error: ${data.error || data.message || 'Auth Failed'}`);
      passedTests++;
    } else {
...
async function runBrowserAuthResilienceHarness(validToken, validCustomer) {
  ...
  try {
    ...
    totalTests++;
    await page.goto('https://urbanspaninfra.co.in/portal', { waitUntil: 'domcontentloaded' });
    await page.waitForTimeout(1500);

    const loginTitle = page.locator('text=Urbanspan Client Sign In');
    const isLoginVisible = await loginTitle.count() > 0;
    if (isLoginVisible) {
    ...
    if (bodyContent.length > 50 && (await page.locator('text=Sign In').count() > 0 || await page.locator('header').count() > 0)) {
    ...
    if (validToken && validCustomer) {
      ...
      }, { token: validToken, customer: validCustomer });

      await page.reload({ waitUntil: 'domcontentloaded' });
      await page.waitForTimeout(3000);

      const verifiedBadge = page.locator('text=Verified Client Account');
      const hasVerified = await verifiedBadge.count() > 0;
      const clientName = page.locator(`text=${validCustomer.name}`);
      const hasName = await clientName.count() > 0;

      if (hasVerified && hasName) {
        console.log(`[PASS] Valid Session Portal Loaded: "${validCustomer.name}" with Verified Client Account badge`);
        passedTests++;
      } else {
      ...
      totalTests++;
      const activeContractsTab = page.locator('button:has-text("Active Supply Contracts")');
      if (await activeContractsTab.count() > 0) {
        await activeContractsTab.click();
