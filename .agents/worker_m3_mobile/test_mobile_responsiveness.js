  ...
  try {
    for (const target of TARGETS) {
      ...
      page.on('requestfailed', req => {
        // Ignore expected GA4 or optional trackers
        const url = req.url();
        if (!url.includes('google-analytics') && !url.includes('googletagmanager')) {
          networkFailures.push([Request Failed]   - );
        }
      });

      for (const route of ROUTES) {
        const fullUrl = ${target.baseUrl};
        ...
        try {
          ...
          const bottomBarAudit = await page.evaluate(() => {
            const bar = document.querySelector('.fixed.bottom-0, [class*=pb-safe]');
            if (!bar) return { found: false };
            const rect = bar.getBoundingClientRect();
            const computed = window.getComputedStyle(bar);
            const links = Array.from(bar.querySelectorAll('a, button'));
          ...
          const overflowAudit = await page.evaluate(() => {
            ...
            for (const el of allElements) {
              ...
              // Check if element spills out of 390px viewport
              if (rect.right > docWidth + 2 && rect.width > 0 && window.getComputedStyle(el).overflowX !== 'auto' && window.getComputedStyle(el).overflowX !== 'scroll') {
                ...
                // Filter out hidden or parent elements that contain allowed scroll containers
          ...
          if (overflowAudit.hasHorizontalOverflow) {
            ...
            routeAudit.issues.push(Horizontal scroll overflow detected: scrollWidth=px > px);
          }
          if (bottomBarAudit.found && !bottomBarAudit.allTouchTargetsValid) {
            routeAudit.passed = false;
            routeAudit.issues.push(Bottom tab bar touch targets invalid: );
          }
          if (consoleErrors.length > 0) {
            routeAudit.issues.push(Console errors encountered: );
          }

          if (routeAudit.passed) {
            targetResult.summary.passed++;
            console.log(  [PASS]  () - 0 Overflow, Bottom Bar 6 tabs OK);
          } else {
          ...
          routeAudit.passed = false;
          routeAudit.issues.push(Navigation / Evaluation error: );
          targetResult.summary.failed++;
          console.log(  [ERR]   () - );
        }
    ...
    console.log(\nAudit completed! Detailed results saved to: );
    return results;
